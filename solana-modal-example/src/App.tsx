...
function App() {
  ...
  async function createVault() {
    ...
    const reponse = await axios.post('http://localhost:3333/create-vault')
    const { vaultId } = reponse.data;
    setVaultId(vaultId);
  }
  ...
  async function deposit() {
    ...
    const rpc = new RPC(provider);
    const [address] = await rpc.getAccounts();
    const reponse = await axios.post(`http://localhost:3333/deposit/${vaultId}`, {
      address: address,
    })
    const { serializedTransaction } = reponse.data;
    ...
    const reponse2 = await axios.post(`http://localhost:3333/execute`, {
      vaultId: vaultId,
      transactionId: reponse.data.transactionId,
  ...
  const loggedInView = (
    <>
      ...
      <div className="flex-container">
        <div>
          <button onClick={createVault} className="card">
            create vault
          </button>
        </div>
        <div>
          <button onClick={deposit} className="card">
            deposit
          </button>
        </div>
      </div>
